...
5. **Fold it back into the repo, at the file this reconcile was for** - the write target depends on which instruction file triggered this branch, not always `CLAUDE.md`:
   - **Claude Code / Cowork** (reconciling `~/.claude/CLAUDE.md` or `~/Documents/CLAUDE.md`) - write the merged result to `[local_path]/CLAUDE.md`.
   - **Codex** (reconciling `$CODEX_HOME/AGENTS.md`, per Tier 1 step b below) - write the merged result to `[local_path]/adapters/codex.md`, **not** `CLAUDE.md`. `adapters/codex.md` is the repo's source for `AGENTS.md`; folding an AGENTS-shaped merge into `CLAUDE.md` would clobber the Claude persona file with Codex-shaped content instead of updating the file it actually came from.

   Then run the **safety gate from `sync-profile` step 2** over the `git_status` file list (refuse anything matching `*.env`/`*.pem`/`*.key`/`*secret*`/`*credential*`/`*token*`/`*.tfstate` or any non-text file), then - **after confirming with the user, because this pushes to their remote** - `git_commit_push` with a message like `cortex: reconcile local profile on restore`. If the user declines the push, skip it; the destination file is already correct, and the repo just stays as it was.
