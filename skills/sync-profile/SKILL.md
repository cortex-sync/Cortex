...
- If `git_commit_push` fails because the remote has commits the local branch lacks (a rejected, non-fast-forward push): this device and the remote have both changed since the last sync. Do **not** blindly `git_pull` then retry - `git_commit_push` has already committed this device's changes locally, so a forced pull would discard that commit and the retry would find nothing to push (silent data loss). Run `git_pull` **unforced** first: it is safe and will refuse, confirming the divergence. Cortex sync is last-write-wins and cannot merge, so surface the choice to the user explicitly before doing anything destructive:
  - **Keep the remote version** (this device's unsynced changes are lost): re-run `git_pull` with `force: true`, then re-apply by hand anything still needed and sync again.
  - **Keep this device's version**: there is no force-push tool, so copy the local edits aside, `git_pull` with `force: true` to land on origin, re-apply the edits, then `git_commit_push`.
  Never force either direction without the user's explicit go-ahead.
